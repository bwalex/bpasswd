...
app.GlobalOptView = Backbone.View.extend({
    ...
    saveChanges: function(ev) {
        ...
        localStorage.setItem('bpasswd:global_options', JSON.stringify(this.model.toJSON()));
...
app.GlobalController = GlobalController.extend({
    ...
    saveOpts: function() {
        ...
        localStorage.setItem('bpasswd:salt_options', JSON.stringify(coll));
...
$(function() {
    ...
    var props = [];
    _.each(JSON.parse(localStorage.getItem('bpasswd:salt_options') || "{}"), function(val, key) {
        props.push(_.extend({"name" : key}, val));
    });
    app.optList.reset(props);

    var globalOpts = window.localStorage.getItem('bpasswd:global_options');
    if (globalOpts == null) {
      window.localStorage.setItem('bpasswd:global_options', JSON.stringify({
        "hotkey": "f8",
        "gen_method": "base64",
        "max_len": 32,
        "cost": 6
    ...
    globalOpts = JSON.parse(window.localStorage.getItem('bpasswd:global_options'));

    app.globalOpt.set(globalOpts);
});
