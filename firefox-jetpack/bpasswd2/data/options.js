...
app.GlobalOptView = Backbone.View.extend({
    ...
    saveChanges: function(ev) {
        ...
        self.port.emit('set-pref', 'global_options', this.model.toJSON());
...
app.GlobalController = GlobalController.extend({
    ...
    saveOpts: function() {
        ...
        self.port.emit('set-pref', 'salt_options', coll);
...
$(function() {
    ...
    self.port.on("pref-resp", function(p, v) {
        if (p == "salt_options") {
            var props = [];
            _.each(JSON.parse(v), function(val, key) {
                props.push(_.extend({"name" : key}, val));
            });
            app.optList.reset(props);
        } else if (p == "global_options") {
            app.globalOpt.set(JSON.parse(v));
        }
    });

    self.port.emit("get-pref", "salt_options");
    self.port.emit("get-pref", "global_options");
});
